...
use League\CommonMark\Parser\MatchedCharacter;
...
class AbletonMarkdownExtension implements ExtensionInterface
{
    public function register(Environment $environment): void
    {
        ...
        if ($config['enable_rack_embeds'] ?? true) {
        ...
        if ($config['enable_device_refs'] ?? true) {
        ...
        if ($config['enable_parameter_controls'] ?? true) {
        ...
        if ($config['enable_audio_player'] ?? true) {
            $environment->addBlockParser(new AudioPlayerBlockParser());
...
{
    public function getMatchedCharacters(): array
    {
        return ['['];
    }

    public function parse(Cursor $cursor, MatchedCharacter $character): ?AbstractInline
    {
        if (!$cursor->match('/^\[\[rack:([a-f0-9-]{36})(?:\|([^\]]+))?\]\]/i')) {
            return null;
        }
        ...
        return new RackEmbedInline($uuid, $options);
    }
...
class DeviceReferenceParser implements InlineParserInterface
{
    ...
    public function parse(Cursor $cursor, MatchedCharacter $character): ?AbstractInline
    {
        ...
        return new DeviceReferenceInline($deviceName, $parameter);
    }
}
...
class ParameterControlParser implements InlineParserInterface
{
    ...
    public function parse(Cursor $cursor, MatchedCharacter $character): ?AbstractInline
    {
        if (!$cursor->match('/^\{param:([^}|]+)(?:\|min:(\d+))?(?:\|max:(\d+))?(?:\|value:(\d+))?\}/')) {
            return null;
        }
        ...
        return new ParameterControlInline($paramName, $min, $max, $value);
    }
}
        ...
        switch ($preset) {
            ...
            case 'detailed':
                return new HtmlElement('div', ['class' => implode(' ', $classes)], [
                    ...
                    new HtmlElement('div', ['class' => 'rack-embed__description'], 
                        $rack->description ? htmlspecialchars($rack->description) : 'No description available'
                    ),
                    new HtmlElement('div', ['class' => 'rack-embed__devices'], 
            ...
            default: // 'default'
                return new HtmlElement('div', ['class' => implode(' ', $classes)], [
                    new HtmlElement('div', ['class' => 'rack-embed__header'], [
                        new HtmlElement('h4', ['class' => 'rack-embed__title'], 
                            htmlspecialchars($rack->title)
                        ),
                        new HtmlElement('span', ['class' => 'rack-embed__by'], 
                            'by ' . htmlspecialchars($rack->user->name)
                        ),
                    ]),
                    new HtmlElement('div', ['class' => 'rack-embed__meta'], [
                        new HtmlElement('span', ['class' => 'rack-embed__devices'], 
                            $rack->device_count . ' devices'
                        ),
                        new HtmlElement('span', ['class' => 'rack-embed__downloads'], 
...
class AudioPlayerRenderer implements NodeRendererInterface
{
    ...
    public function render($node, ChildNodeRendererInterface $childRenderer)
    {
        ...
        if (!$this->isValidAudioUrl($node->url)) {
            return new HtmlElement('div', ['class' => 'audio-player-error'], 
        ...
        return new HtmlElement('div', [
            ...
            new HtmlElement('div', ['class' => 'audio-player__header'], [
                new HtmlElement('h4', ['class' => 'audio-player__title'], 
